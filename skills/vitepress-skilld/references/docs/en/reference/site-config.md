...
- **Vue**
  ...
  VitePress already includes the official Vue plugin for Vite (@vitejs/plugin-vue). You can configure its options using the [vue](#vue) option in your VitePress config.

- **Markdown**

  You can configure the underlying Markdown-It instance using the [markdown](#markdown) option in your VitePress config.
...
The directory for cache files, relative to [project root](../guide/routing#root-and-source-directory). See also: cacheDir.
