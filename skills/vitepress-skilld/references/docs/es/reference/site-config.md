...
- **Vue**
  ...
  VitePress ya incluye el plugin oficial de Vue para Vite (@vitejs/plugin-vue). Puede configurar sus opciones usando la opción [vue](#vue) en su configuración VitePress.

- **Markdown**

  Puede configurar la instancia subyacente de Markdown-It usando la opción [markdown](#markdown) en su configuración VitePress.
...
Un patrón glob para hacer coincidir los archivos de rebajas que deben exluirse como contenido de origen.
...
El directorio para los archivos de caché, en relación con el [raiz del proyecto](../guide/routing#root-and-source-directory). Vea también: cacheDir.
...
Configure las opciones de procesador Markdown. VitePress usa Markdown-it como procesador y Shiki para resaltar la sintaxis del idioma. Dentro de esta opción, puede pasar varias opciones de Markdown relacionadas para satisfacer sus necesidades.
...
Consulte la declaración de tipo y jsdocs para conocer todas las opciones disponibles.

### vite

- Tipo: `import('vite').UserConfig`

Pase la Configuración Vite sin procesar al servidor interno / empaquetador Vite.
...
Pase las opciones `@vitejs/plugin-vue` sin formato a la instancia del complemento interno.
