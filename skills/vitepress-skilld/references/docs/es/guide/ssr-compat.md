...
La sección SSR en la documentación Vue oficial proporciona más contexto sobre lo que es SSR, la relación entre SSR / SSG y notas comunes sobre escribir código amigable con SSR. La regla general es acceder apenas APIs deln navegador / DOM en los hooks `beforeMount` o `mounted` de los componentes Vue.
...
Puede también importar condicionalmente usando el flag `import.meta.env.SSR` (parte de las variables de entorno Vite):
