...
- **Vue**
  ...
  VitePress já inclui o plugin Vue oficial para Vite (@vitejs/plugin-vue). Você pode configurar suas opções usando a opção [vue](#vue) em sua configuração VitePress.

- **Markdown**

  Você pode configurar a instância subjacente de Markdown-It usando a opção [markdown](#markdown) em sua configuração VitePress.
...
Um padrão glob para corresponder a arquivos markdown que devem ser excluídos como conteúdo fonte.
...
O diretório para arquivos de cache, relativo à [raiz do projeto](../guide/routing#root-and-source-directory). Veja também: cacheDir.
...
Configure as opções do processador Markdown. VitePress usa Markdown-it como processador e Shiki para destacar sintaxe de linguagem. Dentro desta opção, você pode passar várias opções Markdown relacionadas para atender às suas necessidades.
...
Verifique a declaração de tipo e jsdocs para todas as opções disponíveis.

### vite

- Tipo: `import('vite').UserConfig`

Passe a Configuração Vite crua para o servidor interno / empacotador Vite.
...
Passe as opções `@vitejs/plugin-vue` cruas para a instância interna do plugin.
