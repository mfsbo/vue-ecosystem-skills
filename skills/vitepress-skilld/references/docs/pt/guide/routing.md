...
- Vercel requer a ativação da opção `cleanUrls` no `vercel.json`.
...
Os caminhos de reescrita são compilados usando o pacote `path-to-regexp` - consulte sua documentação para uma sintaxe mais avançada.
