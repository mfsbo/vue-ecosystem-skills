...
- **Vue**
  ...
  VitePress 已经包含 Vite 的官方 Vue 插件 (@vitejs/plugin-vue)，所以我们可以配置 VitePress 中的 [vue](#vue) 选项。

- **Markdown**

  可以使用 VitePress 配置中的 [markdown](#markdown) 选项配置底层的 Markdown-It 实例。
...
缓存文件的目录，相对于[项目根目录](../guide/routing#root-and-source-directory)。另请参阅：cacheDir。
