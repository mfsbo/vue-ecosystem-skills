...
export const LeafletDoc = (props) => (
  <DocWrapper {...leafletMapProps() } {...props} />
)
...
(like MapTiler Cloud, which has a free tier) or host your own tiles
(we've successfully used OpenMapTiles in the past).


Let's assume you've got an API key from MapTiler. You can initialize _LeafletMap_ by providing a `style` URL or
object to it. MapTiler has a few vector styles available by default, but you can
...
useRef hook to access the Typescript instance of
_LeafletMap_. In Angular there's a ViewChild decorator allowing you to do the same.
...
## Using Raster files
By default, _LeafletMap_ uses the MapLibre GL JS renderer and
...
can MapLibre's `StyleSpecification` object
be used.
:::
...
{() => {
  ...
  <LeafletDoc
    ...
    declarations={{ ...leafletMapProps().declarations, tooltip: declaration }}
    showContext={ContextLevel.Minimal}
