...
unpkg.comはnpmに基づいたCDNリンクを提供します。上記リンクは常にnpmの最新版へのリンクです。
...
## 異なるビルドの説明
dist/ directory of the npm package内に、Vue I18nの多くの異なるビルドがあります。目的に応じてどのdistファイルを使うべきかの概要をここに示します。
...
:::tip 備考
グローバルビルドはUMDビルドではありません。これらはIIFEsとしてビルドされ、`<script src="...">` で直接使用することのみを意図しています。
:::
