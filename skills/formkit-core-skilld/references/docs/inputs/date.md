...
A native HTML date picker.

The `date` input uses HTML's native date picker to allow users to easily select a date.

> **Tip:** Need more flexibility than the native HTML date input provides? Check out the datepicker input available in FormKit Pro.
...
> **Warning:** The internal format of all native date pickers is YYYY-MM-DD. This is true even though the format of the date displayed to the user may be different. For example U.S. based browsers display date format as MM/DD/YYYY.
...
Additional slot data specific to the `date` input:
...
| Key | Description |
|-----|-------------|
| `enter` | Opens the browser date picker panel when input is focused. Confirms date selection when date picker is focused. |
| `space` | Opens the browser date picker panel when input is focused. |
