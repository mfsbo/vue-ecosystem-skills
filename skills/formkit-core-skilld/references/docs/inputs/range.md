...
A native HTML range input.

The `range` input uses HTML's native range input to let user select a number with a graphic slider.

> **Tip:** Need more flexibility than the native HTML range input provides? Check out the slider input available in FormKit Pro.
...
Additional slot data specific to the `range` input:
...
| Key | Description |
|-----|-------------|
| `up` | Adjust the input value up by the current step. |
| `right` | Adjust the input value up by the current step. |
| `left` | Adjust the input value down by the current step. |
| `down` | Adjust the input value down by the current step. |
| `PageUp` | Adjust the input value up by 10x the current step. |
| `PageDown` | Adjust the input value down by 10x the current step. |
| `Home` | Set the input value to the min value. |
| `End` | Set the input value to the max value. |
