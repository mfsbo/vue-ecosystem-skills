...
> **Warning:** The internal format of all native time pickers is HH:MM. This is true even though the format of the time displayed to the user may be 12 hour or 24 hour.
...
Additional slot data specific to the `time` input:
...
| Key | Description |
|-----|-------------|
| `enter` | Opens the browser time picker panel when input is focused. Confirms time selection when time picker is focused. |
| `space` | Opens the browser time picker panel when input is focused. |
| `esc` | Closes the browser time picker panel. |
| `up` + `down` + `left` + `right` | Adjusts selection of UI elements in browser time picker panel. |
