...
A native HTML month picker.

The `month` input uses HTML's native month picker to allow users to easily select a month.

> **Danger:** Month inputs are not currently supported in FireFox or Safari.
...
> **Warning:** The internal format of all native month pickers is YYYY-MM. This is true
even though the format of the month displayed to the user may be different. For example, U.S. based browsers display the month format as Month Name YYYY.
...
Additional slot data specific to the `month` input:
...
| Key | Description |
|-----|-------------|
| `enter` | Opens the browser month picker panel when input is focused. Confirms month selection when month picker is focused. |
| `space` | Opens the browser month picker panel when input is focused. |
| `esc` | Closes the browser month picker panel. |
| `up` + `down` + `left` + `right` | Adjusts selection of UI elements in browser month picker panel. |
