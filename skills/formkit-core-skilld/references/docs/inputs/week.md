...
A native HTML week picker.

The `week` input uses the HTML's native week picker to allow users to easily
select a week.
...
> **Danger:** Week inputs are not currently supported in FireFox or Safari.

> **Warning:** The internal format of all native week pickers is YYYY-Www (for example: 2017-W06). This is true even though the format of the week displayed to the user may be different..
...
Additional slot data specific to the `week` input:
...
| Key | Description |
|-----|-------------|
| `enter` | Opens the browser week picker panel when input is focused. Confirms week selection when week picker is focused. |
| `space` | Opens the browser week picker panel when input is focused. |
| `esc` | Closes the browser week picker panel. |
| `up` + `down` + `left` + `right` | Adjusts selection of UI elements in browser week picker panel. |
