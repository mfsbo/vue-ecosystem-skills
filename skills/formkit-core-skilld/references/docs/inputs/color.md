...
Additional slot data specific to the `color` input:
...
| Key | Description |
|-----|-------------|
| `enter` | Opens the browser color picker panel when input is focused. Confirms color selection when color picker is focused. |
| `space` | Opens the browser color picker panel when input is focused. |
| `esc` | Closes the browser color picker panel. |
| `up` + `down` + `left` + `right` | Adjusts selection of UI elements in browser color picker panel. |
