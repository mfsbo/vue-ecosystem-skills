...
A native HTML button element used in place of a native HTML submit input.
...
> **Info:** Type submit input uses a button element instead of an input=type"submit" because an input is a void element. As a container element, a button can include content and pseudo elements — making them the more flexible option for developers.
...
| `default` | (none) | The default slot for the button label/content. |
...
| Key | Description |
|-----|-------------|
| `enter` | Invokes the submit input when focused. |
