...
`NavigationResult` is a class that can be returned or thrown from a loader to _change_ the navigation. It accepts the same arguments as the return value of a navigation guard **as long as it changes the navigation**. It doesn't accept `true` or `undefined` as these values do not change the navigation.
